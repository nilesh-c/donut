...
from hilayout_acc._modules import CustomT5Config, SpatialEmbeddings, RetrievalModule
...
class Proxy_HiLT5(nn.Module):
    ...
    def __init__(self, config):
        ...
        t5_model_name_or_path = config.get("t5_model_name_or_path", "t5-base")
        self.page_tokens = config.get('page_tokens', 20)
        self.max_doc_pages = config.get('max_pages', 20)

        config_x = CustomT5Config.from_pretrained(t5_model_name_or_path)
        ...
        self.tokenizer = T5Tokenizer.from_pretrained(t5_model_name_or_path)
        ...
        self.model = HiLT5.from_pretrained(t5_model_name_or_path, config=config_x, ignore_mismatched_sizes=True)
    ...
    def forward(self, batch, output_attentions=False, return_pred_parse=False, return_json=False):
        ...
        for batch_idx in range(bs):
            for page_idx in range(num_pages[batch_idx]):
                if len(words[batch_idx][page_idx]) >= 1 and len(boxes[batch_idx][page_idx]) >= 1:
                ...
                # trim context_boxes to the longest sequence just to be safe. Got this error:
                # RuntimeError: The expanded size of the tensor (791) must match the existing size (792) at non-singleton dimension 0.  Target sizes: [791, 4].  Tensor sizes: [792, 4]
                
                all_boxes[batch_idx, page_idx, self.page_tokens: self.page_tokens + len(context_boxes)] = context_boxes[:longest_sequence]
