...
class LayoutTransformerDataset(Dataset):
    ...
    def __init__(
        ...
        self._generate_token_sequences()
    ...
    def _generate_token_sequences(self):
        # pickle gt_token_sequences and tokenizer additional_special_tokens, so that we don't have to recompute them every time
        cache_path = os.path.join(self.dataset_name_or_path, f"cache_gt_tokens_singlepage_{self.split}.pkl")
        if self.use_cache and os.path.exists(cache_path):
            print(f"Loading tokenized gt parses from cache at {cache_path}")
            with open(cache_path, "rb") as f:
                self.gt_token_sequences, additional_special_tokens = pickle.load(f)
                self.model.add_special_tokens(additional_special_tokens)
        else:
            self.gt_token_sequences = []
            for sample in tqdm(self.dataset, desc="Converting json parses to token sequences for split {}".format(self.split), dynamic_ncols=True, position=0, unit="samples"):
                ...
                if (
                    ...
                    for gt_json in gt_jsons:
                        if "workexperience" in gt_json:
                            del gt_json["workexperience"]
                        if "education" in gt_json:
                            del gt_json["education"]
            ...
            if self.use_cache:
                # save gt_token_sequences and tokenizer additional_special_tokens to cache
                print(f"Saving tokenized gt parses to cache at {cache_path}")
                with open(cache_path, "wb") as f:
                    additional_special_tokens = self.model.tokenizer.additional_special_tokens
                    pickle.dump((self.gt_token_sequences, additional_special_tokens), f)
    ...
    def __getitem__(
        ...
        num_pages = min(len(sample["page_names"]), self.max_pages, len(sample['tokens']))
        
        if num_pages < 1:
            raise Exception(f"num_pages < 1 for {sample['doc_name']}")
        ...
        for page_ix in range(num_pages):
            word_list = [token["token_text"] for token in sample['tokens'][page_ix]]
            words.append(word_list)
            boxes.append(np.array(self._get_boxes(sample['tokens'][page_ix], page_size), dtype=np.float32))
            context.append(' '.join(word_list))
    ...
    def load_dataset_multipage(cls, dataset_name_or_path, split):
        cache_path = os.path.join(dataset_name_or_path, split, f"metadata.jsonl.pkl")
        if os.path.exists(cache_path):
            print(f"Loading dataset from cache at {cache_path}")
            with open(cache_path, "rb") as f:
            ...
            with jsonlines.open(json_path) as reader:
                # reader = itertools.islice(reader, 128)
                reader = tqdm(reader, desc=f"Loading dataset {dataset_name_or_path} split {split}", dynamic_ncols=True, position=0, unit="samples")
