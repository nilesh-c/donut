...
from hilayout.lightning_module import LayoutTransformerModelPLModule, LayoutTransformerDataPLModule
from hilayout.util import LayoutTransformerDataset
...
class CustomCheckpointIO(CheckpointIO):
    ...
    def load_checkpoint(self, path, storage_options=None):
        checkpoint = torch.load(Path(path) / "artifacts.ckpt")
        return checkpoint
...
def train(config):
    ...
    if config.load_from_checkpoint:
        model_module = LayoutTransformerModelPLModule.load_from_checkpoint(Path(config.pretrained_model_name_or_path) / "artifacts.ckpt", config=config)
    else:
